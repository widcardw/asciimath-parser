...
import { flat, frac, lp, mat, opOA, opOAB, root, rp, sup, u } from './utils/build-node'
  ...
  it('should parse inline array', () => {
    expect(removeValue(parser(trie.tryParsingAll('A=[a,b,c,d]')))).toEqual(
      root(
        ...
        flat(
          lp('['),
          u('a'), u(','), u('b'), u(','), u('c'), u(','), u('d'),
          rp(']'),
        ),
      ))
  })
  it('should parse a recursive matirx', () => {
    expect(removeValue(parser(trie.tryParsingAll('B = [[a,b],c;d,e]')))).toEqual(
      root(
        u('B'), u('='), mat([
          [
...
describe('cases matrix', () => {
  ...
  it('should parse cases', () => {
    expect(removeValue(parser(trie.tryParsingAll('f(x)={x^2, if x>0; x, otherwise:}')))).toEqual(
      root(
        u('f'), flat(lp('('), u('x'), rp(')')), u('='),
        mat([
          [
...
describe('parse det', () => {
  ...
  it('should parse a set expression', () => {
    expect(removeValue(parser(trie.tryParsingAll('{(x,y)|x^2+y^2<=1}')))).toEqual(
      root(flat(
        lp('{'),
        flat(
          lp('('),
  ...
  it('should parse a det with inner parens', () => {
    expect(removeValue(parser(trie.tryParsingAll('|1,2;(3 + 4)^7,4|')))).toEqual(
      root(mat([
        [flat(u('1')), flat(u('2'))],
        [
          flat(
...
describe('parse aligned expressions', () => {
  ...
  it('should parse an aligend expression of 2 lines', () => {
    expect(removeValue(parser(trie.tryParsingAll(CODE)))).toEqual(
      root(
        u('x'), u('&'), u('\\mapsto'), u('\\text{e}'), u('x'),
        u('\\\\'),
        u('f'), flat(lp('('), u('x'), rp(')')), u('&'), u('\\to'), u('g'), flat(lp('('), u('x'), rp(')')),
      ),
    )
  })
})
...
describe('parse expression like sup and sub', () => {
  ...
  it('should parse a complex sup expression', () => {
    expect(removeValue(parser(trie.tryParsingAll('"e"^(x + 2x + 4x^2)')))).toEqual(
      root(flat(
        u('\\text{e}'),
        sup('^', flat(
          u('x'),
...
describe('parse operator A op B', () => {
  ...
  it('should parse more `/`', () => {
    ...
    expect(removeValue(parser(trie.tryParsingAll('(a + c)/(b / d + e)')))).toEqual(
      root(
        frac(
          flat(u('a'), u('+'), u('c')),
          flat(
  ...
  it('should parse single right paren', () => {
    expect(removeValue(parser(trie.tryParsingAll(')')))).toEqual(
      root(u(')')),
    )
  })
})
